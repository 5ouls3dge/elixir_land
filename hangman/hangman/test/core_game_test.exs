defmodule HangmanCoreGameTest do
  use ExUnit.Case
  ...
  test "new game returns correct word" do
    game = Game.new_game("radha")

    assert game.turns_left == 7
    assert game.game_state == :initializing
    assert length(game.letters) > 0
    assert game.letters == ["r", "a", "d", "h", "a"]
  end

  test "state dosnt change if a game is won or lost" do
  ...
  test "state dosnt change if a game is won" do
      game = Game.new_game("radha")
  ...
  test "a dulicated letter is reported" do
    game = Game.new_game()
  ...
  test "we recognize a latter in the word" do
    {_game, tally} = Game.make_move(Game.new_game("radha"), "a")
    assert tally.game_state == :good_guess
    {_game, tally} = Game.make_move(Game.new_game("radha"), "h")
    assert tally.game_state == :good_guess
  end
